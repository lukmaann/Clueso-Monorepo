            ...
            <div className="flex flex-col gap-10">
                ...
                <div className="w-full bg-slate-900 rounded-2xl overflow-hidden shadow-2xl ring-1 ring-black/5 relative group">
                    <div className="aspect-video relative mx-auto max-w-5xl bg-black">
                        ...
                        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent p-6 pt-16 flex items-center gap-6 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => {
                                if (videoRef.current?.paused) { videoRef.current.play(); setIsPlaying(true); }
                                else { videoRef.current?.pause(); setIsPlaying(false); }
                            ...
                            <div className="flex-1">
                                <div className="h-1 bg-white/20 rounded-full overflow-hidden">
                                    <div className="h-full bg-indigo-500 w-0 transition-all duration-300" style={{ width: videoRef.current ? `${(videoRef.current.currentTime / videoRef.current.duration) * 100}%` : '0%' }}></div>
                                </div>
                            </div>

                            <div className="flex items-center gap-4 text-white/80 text-sm font-medium">
                                <span className="flex items-center gap-2">
                                    {audioMode === 'ai' ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
                                    {audioMode === 'ai' ? 'Video Muted' : 'Video Audio'}
                                </span>
                            </div>
                        </div>
                    </div>
